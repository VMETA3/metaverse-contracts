...
const TokenURI = '{"name":"elf 7","description":"this is the 7th elf!","price":"0.09","image":"https://gateway.pinata.cloud/ipfs/QmNzNDMzrVduVrQAvJrp8GwdifEKiQmY1gSfPbq12C8Mhy"}';
...
describe('Land Token', () => {
  ...
  describe('mint land', async () => {
    it('Simulate a scenario that the user creates at will', async () => {
      ...
      const User = users[10];
      await expect(User.Proxy.awardItem(User.address, TestConditions, TokenURI)).to.revertedWith("SafeOwnableUpgradeable: caller is not the owner");
    });
    it('Admin build land', async () => {
      ...
      await expect(Administrator1.Proxy.awardItem(User.address, TestConditions, TokenURI))
        .to.emit(Proxy, 'Transfer').withArgs(ZeroAddr, User.address, 0);
    });
  });
  ...
  describe('inject active', async () => {
    it('Called in an unauthorized state', async () => {
      ...
      expect(await Proxy.ownerOf(0)).to.eq(Administrator1.address);

      await expect(User.Proxy.injectActive(TokenID, active, nonce)).to.revertedWith("SafeOwnableUpgradeable: operation not in pending");

      const refundHash = web3.utils.hexToBytes(await User.Proxy.getInjectActiveHash(TokenID, active, User.address, nonce));
      ...
      await Administrator1.Proxy.AddOpHashToPending(refundHash, [Sig2]);

      await expect(await User.Proxy.injectActive(TokenID, active, nonce))
        .to.emit(Proxy, 'Activation')
        .withArgs(User.address, active, false);
    });
  })
});

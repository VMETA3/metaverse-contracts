...
describe('Vip testing', () => {
    it('multiple Invest within 30 days', async () => {
        ...
        const currentNumbers = [VipLevelList[0].currentNumber, VipLevelList[1].currentNumber, VipLevelList[2].currentNumber];
    ...
    it('the vip is full and cannot be invested', async () => {
        ...
        await expect(testUser1.Proxy.deposit(amount))
            .to.be.revertedWith('Vip: exceed the number of people limit');
    });
    ...
    it('get contract information', async () => {
        ...
        expect((await testUser1.Proxy.getLatestList()).toString()).to.be.eq(`${testUser1.address},1,${testUser2.address},2,${testUser3.address},3`);

        // check getLevelArray function
        expect((await testUser1.Proxy.getLevelArray()).toString()).to.be.eq(`${VipLevelList[0].level},${VipLevelList[0].threshold},${VipLevelList[0].numberLimit},1,${VipLevelList[1].level},${VipLevelList[1].threshold},${VipLevelList[1].numberLimit},1,${VipLevelList[2].level},${VipLevelList[2].threshold},${VipLevelList[2].numberLimit},1`);
    });
});
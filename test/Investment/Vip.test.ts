...
const setup = deployments.createFixture(async () => {
    ...
    const VipProxy = await upgrades.deployProxy(Vip, [
        owners,
        signRequired,
    ]);
...
describe('Vip testing', () => {
    it('multiple Invest in a month', async () => {
        ...
        await Administrator1.Proxy.setLv1NumberLimit(2000);
        await Administrator1.Proxy.setLv2NumberLimit(700);
        await Administrator1.Proxy.setLv3NumberLimit(150);
        await Administrator1.Proxy.setL1Threshold(100);
        await Administrator1.Proxy.setL2Threshold(1000);
        await Administrator1.Proxy.setL3Threshold(10000);
        ...
        // deposit amout, index 0 expect level 1
        ...
        expect(await testUser1.Proxy.getLevel(0)).to.be.eq(levelOne.Level);

        // deposit amout2, index 0 expect level 2
        const levelTwo = VipLevelList[1];
        await expect(testUser1.Proxy.deposit(amount2))
            .to.be.emit(Proxy, 'Deposit')
            .withArgs(testUser1.address, amount2);
        expect(await testUser1.Proxy.getLevel(0)).to.be.eq(levelTwo.Level);

        // deposit amout3, index 0 expect level 3
        const levelThree = VipLevelList[2];
        await expect(testUser1.Proxy.deposit(amount3))
            .to.be.emit(Proxy, 'Deposit')
            .withArgs(testUser1.address, amount3);
        expect(await testUser1.Proxy.getLevel(0)).to.be.eq(levelThree.Level);
    });

    it('multiple Invest more than a month', async () => {
        const { Proxy, ERC20Token, deployer, Administrator1, users } = await setup();
        // init
        await Administrator1.Proxy.setERC20(ERC20Token.address);
        await Administrator1.Proxy.setSpender(deployer.address);
        ...
        // a month later
        await Administrator1.Proxy.setCurrentTime(startTime + SecondsForMonth + SecondsForDay);

        // deposit amout2, index 1 expect level 2
        const levelTwo = VipLevelList[1];
        await expect(testUser1.Proxy.deposit(amount2))
            .to.be.emit(Proxy, 'Deposit')
            .withArgs(testUser1.address, amount2);
        expect(await testUser1.Proxy.getLevel(1)).to.be.eq(levelTwo.Level);

        // a month later
        await Administrator1.Proxy.setCurrentTime(startTime + (SecondsForMonth * 2) + SecondsForDay);

        // deposit amout3, index 2 expect level 3
        const levelThree = VipLevelList[2];
        await expect(testUser1.Proxy.deposit(amount3))
            .to.be.emit(Proxy, 'Deposit')
            .withArgs(testUser1.address, amount3);
        expect(await testUser1.Proxy.getLevel(2)).to.be.eq(levelThree.Level);
    });
});
